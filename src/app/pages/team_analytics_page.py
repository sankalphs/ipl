...
from src.app.cache import load_data, load_registry
...
def render():
    ...
    team = st.selectbox("Select Team", all_teams, key="ta_team")

    # Overall record
    st.subheader(f"{team} - Overall Record")
    team_matches = matches[
        ((matches["team1"] == team) | (matches["team2"] == team)) & (matches["has_result"] == True)
    ]
    ...
    for season in sorted(matches["season"].unique()):
        ...
        season_stats.append({
            "Season": season, "Matches": len(season_matches),
            "Wins": season_wins, "Losses": len(season_matches) - season_wins,
    ...
    if season_stats:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=season_df["Season"], y=season_df["Wins"], name="Wins", marker_color="green"))
        fig.add_trace(go.Bar(x=season_df["Season"], y=season_df["Losses"], name="Losses", marker_color="red"))
        fig.update_layout(barmode="stack", title=f"{team} - Season-wise Results",
                          xaxis_title="Season", yaxis_title="Matches", height=400)
        st.plotly_chart(fig, width="stretch")

        fig2 = px.line(season_df, x="Season", y="Win Rate", title=f"{team} - Win Rate Trend", markers=True)
    ...
    h2h_opponent = st.selectbox("Opponent", [t for t in all_teams if t != team], key="ta_h2h_opp")
    ...
    if h2h_total > 0:
        h2h_venue = h2h_matches.groupby("venue").apply(
            lambda x: pd.Series({"matches": len(x), f"{team}_wins": (x["winner"] == team).sum()})
        ).reset_index().sort_values("matches", ascending=False).head(10)
        st.dataframe(h2h_venue, width="stretch")
    ...
    for venue in team_matches["venue"].dropna().unique():
        ...
        venue_stats.append({
            "Venue": venue, "Matches": len(venue_m), "Wins": v_wins,
    ...
    st.dataframe(venue_df.head(15), width="stretch")
    ...
    if len(impact_matches) > 0:
        ...
        if len(pre_impact) > 0:
            ...
            with c2:
                ...
                st.metric("Post-2023 Win Rate", f"{impact_wins/len(impact_matches)*100:.1f}%", delta=f"{diff:+.1f}%")
    else:
        st.info("No 2023+ matches found for this team.")
    ...
    for _, match in team_matches.iterrows():
        participants = registry.get_match_players(match["match_id"])
        for p in participants.get(team, []):
            team_players[p] = team_players.get(p, 0) + 1

    if team_players:
        top_players = sorted(team_players.items(), key=lambda x: x[1], reverse=True)[:20]
        st.dataframe(pd.DataFrame(top_players, columns=["Player", "Matches"]), width="stretch")
